...
import { ConsultantAdvanceStageButton } from "@/components/ConsultantAdvanceStageButton";
import { ConsultantCaseDocUploadForm } from "@/components/ConsultantCaseDocUploadForm";
...
export default async function ConsultantCaseDetailPage({ params }: { params: Promise<{ caseId: string }> }) {
  ...
  if (!theCase || theCase.consultantId !== user.id) notFound();
  ...
  return (
    <div>
      <AppHeader title="Case Detail" backHref="/consultant/pipeline" />
      <div className="px-4 pt-4">
        ...
        {nextLabel && <ConsultantAdvanceStageButton caseId={theCase.id} label={nextLabel} />}
        ...
        <ConsultantCaseDocUploadForm caseId={theCase.id} />
